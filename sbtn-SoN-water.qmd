...
```{r}
bws <- st_read(file.path(sharepoint_dir, "ws_gis", "1_library", "aqueduct_4.0", "aqueduct-4-0-water-risk-data", "Aqueduct40_waterrisk_download_Y2023M07D05", "GDB", "Aq40_Y2023D07M05.gdb"), layer = "baseline_annual")
...
```{r}
wdp <- rast(file.path(sharepoint_dir, "ws_gis", "1_library", "WaterGap3", "WaterDepletion", "WaterDepletionCat_WG3.tif"))
...
'FB-OuR-Daten - WRF\ws\_gis\\1_library\BlueWaterScarcity\WS\_blue_monthly_rasters\WSbl\_monthly_30m'

```{r}
wsb <- rast(file.path(sharepoint_dir, "ws_gis", "1_library", "BlueWaterScarcity", "WS_blue_monthly_rasters", "WSbl_monthly_30m", "ws_avg.tif"))
...
```{r}
nox_csv <- read_csv(file.path(sharepoint_dir, "ws_gis", "1_library", "WorldBank_ML_Quality", "noxn_preds_05_16_19.csv"))
...
```{r}
sp_grid <- read_csv(file.path(sharepoint_dir, "ws_gis", "1_library", "WorldBank_ML_Quality", "ML_data_gridcell_match.csv"))
...
```{r}
tnc <- rast(file.path(sharepoint_dir, "ws_gis", "1_library", "water_quality_mcdowell", "data_from_author", "WQ_Variables_Extraction", "TN_Conc.tif"))
...
```{r}
tpc <- rast(file.path(sharepoint_dir, "ws_gis", "1_library", "water_quality_mcdowell", "data_from_author", "WQ_Variables_Extraction", "TP_Conc.tif"))
...
```{r}
wmo_basins <- st_read(file.path(sharepoint_dir, "ws_r", "collective_action_opportunities", "input", "geographies", "wmobb_basins.shp")) |> 
...
```{r}
h6 <- st_read(file.path(sharepoint_dir, "ws_gis", "1_library", "hydrosheds", "hybas_lev06_v1c.shp")) |> 
  select(HYBAS_ID) |>
...
```{r}
write_rds(son_water, here("../outputs", "h6", "sbtn_son_water_v1.1.rds"))
```
