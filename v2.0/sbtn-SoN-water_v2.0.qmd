...
```{r}
tpc <- rast(file.path(params$sharepoint, "10_RiskSuite", "RFS_New_Folders", "05_Data_and_Analysis", "01_Data", "WaterQuality_FreshwaterNutrientPollution_McDowell20", "data_from_author", "WQ_Variables_Extraction", "TP_Conc.tif"))
```
...
```{r}
v1.1 <- read_excel(here("v1.1/outputs/sbtn_son_water_v1.1.xlsx"))
```
...
compare_wa <- son_water |> 
  st_drop_geometry() |> 
  left_join(v1.1 |> 
              select(HYBAS_ID, wa_max_1.1 = wa_max, fqt_raw_1.1 = fqt_raw, bws_raw_1.1 = bws_raw, wdp_raw_1.1 = wdp_raw),
            by = "HYBAS_ID") |> 
  mutate(wa_diff = wa_max - wa_max_1.1)
```
...
```{r}
hist(round(compare_wa$fqt_raw - compare_wa$fqt_raw_1.1, 5))
```

```{r}
hist(round(compare_wa$bws_raw - compare_wa$bws_raw_1.1, 5))
```

```{r}
hist(round(compare_wa$wdp_raw - compare_wa$wdp_raw_1.1, 5))
```
...
compare_wp <- son_water |> 
  st_drop_geometry() |> 
  left_join(v1.1 |> 
              select(HYBAS_ID, wp_max_1.1 = wp_max, cep_raw_1.1 = cep_raw, nox_raw_1.1 = nox_raw, tnc_raw_1.1 = tnc_raw, tpc_raw_1.1 = tpc_raw),
            by = "HYBAS_ID") |> 
  mutate(wp_diff = wp_max - wp_max_1.1) |> 
  arrange(desc(wp_diff)) |> 
  select(HYBAS_ID, starts_with("wp_"), cep_raw, cep_raw_1.1, nox_raw, nox_raw_1.1, tnc_raw, tnc_raw_1.1, tpc_raw, tpc_raw_1.1)

compare_wp
```

```{r}
hist(round(compare_wp$cep_raw - compare_wp$cep_raw_1.1, 5))
```

```{r}
hist(round(compare_wp$nox_raw - compare_wp$nox_raw_1.1, 5))
```

```{r}
hist(round(compare_wp$tnc_raw - compare_wp$tnc_raw_1.1, 5))
```

```{r}
hist(round(compare_wp$tpc_raw - compare_wp$tpc_raw_1.1, 5))
```
