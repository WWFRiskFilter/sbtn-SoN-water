---
title:  "SBTN State of Nature Water unified layers (v2.0)"
...
```{r}
fqt <- read_sf(file.path(params$sharepoint, "10_RiskSuite", "RFS_New_Folders", "05_Data_and_Analysis", "01_Data", "WaterAvailability_FreshwaterQuantityTargets_Hogeboom24", "v1.0", "Freshwater Quantity Targets.shp"))
```
...
```{r}
if (params$overwrite | !file.exists(here("./v2.0/outputs", "h6.rds"))){
  ...
  write_rds(h6, here("./v2.0/outputs", "h6.rds")) 
} else {
  h6 <- read_rds(here("./v2.0/outputs", "h6.rds"))
}
```
...
```{r}
if (params$overwrite | !file.exists(here("./v2.0/outputs", "wa_n.rds"))) {
  wa_n <- h6 |>
    st_join(bws_n, largest = TRUE) |>
    left_join(wdp_n, by = "HYBAS_ID") |>
    st_join(fqt_n, largest = TRUE) 
  write_rds(wa_n, here("./v2.0/outputs", "wa_n.rds"))
} else {
  wa_n <- read_rds(here("./v2.0/outputs", "wa_n.rds"))
}
```
...
```{r}
if(params$overwrite | !file.exists(here("./v2.0/outputs", "wp_n.rds"))){
 wp_n <- h6 |>
  st_join(cep_n, largest = TRUE) |>
  left_join(nox_n, by = "HYBAS_ID") |>
  left_join(pgp_n, by = "HYBAS_ID")
 write_rds(wp_n, here("./v2.0/outputs", "wp_n.rds"))
} else {
  wp_n <- read_rds(here("./v2.0/outputs", "wp_n.rds"))
}
```
...
We want to check: - In v2.0, which dataset is driving risk score in each location?
i.e. which dataset has the max value?
...
ggsave(plot = BWS_driver, filename = here("./v2.0/outputs", "BWS_driver.png"))
```
...
ggsave(plot = WDP_driver, filename = here("./v2.0/outputs", "WDP_driver.png"))
```
...
ggsave(plot = FQT_driver, filename = here("./v2.0/outputs", "FQT_driver.png"))
```
...
ggsave(plot = top_model_plot, filename = here("./v2.0/outputs", "all_drivers.png"))
```
...
ggsave(plot = top_model_plot_highrisk, filename = here("./v2.0/outputs", "all_drivers_highrisk.png"))
```

## 7. Export

```{r}
write_rds(son_water, here("./v2.0/outputs", "sbtn_son_water_v2.0.rds"))
```

```{r}
st_write(son_water, here("./v2.0/outputs", "sbtn_son_water_v2.0.shp"), layer_options = "ENCODING=UTF-8", delete_layer = TRUE)
```

```{r}
write.xlsx(list(
  "SBTN SoN Water" = st_drop_geometry(son_water)
), file = here("./v2.0/outputs", "sbtn_son_water_v2.0.xlsx"), rowNames = FALSE)
```
